...
@php
    ...
    $isDashboard = $url === filament()->getUrl();
@endphp
...
<li
    ...
    <a {{ \Filament\Support\generate_href_html($url, $shouldOpenUrlInNewTab) }}
        ...
        @if ($isDashboard) wire:current.exact="bg-zinc-800 fi-active" @else wire:current.relative="bg-zinc-800 fi-active" @endif
        ...
        @if (filled($icon) && (!$subGrouped || ($sidebarCollapsible && !$subNavigation)))
            {{ \Filament\Support\generate_icon_html(
                ...
                attributes: new \Illuminate\View\ComponentAttributeBag([
                    ...
                    $isDashboard ? 'wire:current.exact' : 'wire:current.relative' => 'text-white',
                    'href' => $url,
        ...
        @if ((blank($icon) && $grouped) || $subGrouped)
            <div @if (filled($icon) && $subGrouped && $sidebarCollapsible && !$subNavigation) x-show="$store.sidebar.isOpen" @endif
        ...
        <span
            ...
            @if ($isDashboard) wire:current.exact="text-white" @else wire:current.relative="text-white" @endif
            href="{{ $url }}" class="fi-sidebar-item-label">
            {{ $slot }}
        </span>
    ...
    @if ($childItems)
        <ul x-show="$el.closest('.fi-sidebar-item').querySelector('.fi-active')" class="fi-sidebar-sub-group-items">
            @foreach ($childItems as $childItem)
                ...
                <x-prasmanan::filament-panels.sidebar.item :active="$isChildActive" :active-child-items="$isChildItemChildItemsActive" :active-icon="$childItemActiveIcon" :badge="$childItemBadge"
                    ...
                    {{ $childItem->getLabel() }}
                </x-prasmanan::filament-panels.sidebar.item>
            @endforeach
        </ul>
    @endif
</li>
