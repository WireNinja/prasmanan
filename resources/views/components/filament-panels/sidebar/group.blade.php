...
<li x-data="{ label: @js($subNavigation ? "sub_navigation_{$label}" : $label) }" data-group-label="{{ $subNavigation ? "sub_navigation_{$label}" : $label }}"
    x-bind:class="{
        'fi-collapsed': $store.sidebar.groupIsCollapsed(label),
        'fi-active': $el.querySelector('.fi-active'),
    }"
    {{ $attributes->class(['fi-sidebar-group', 'fi-collapsible' => $collapsible]) }}>
    @if ($label)
    ...
    @if ($hasDropdown)
        <x-filament::dropdown :placement="(__('filament-panels::layout.direction') === 'rtl') ? 'left-start' : 'right-start'" x-show="! $store.sidebar.isOpen">
            <x-slot name="trigger">
