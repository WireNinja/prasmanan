...
<x-filament::dropdown :placement="$position === UserMenuPosition::Topbar ? 'bottom-end' : 'top-end'" :teleport="$position === UserMenuPosition::Topbar" :attributes="\Filament\Support\prepare_inherited_attributes($attributes)->class(['fi-user-menu'])">
    <x-slot name="trigger">
        @if ($position === 'left-rail')
            <button aria-label="{{ __('filament-panels::layout.actions.open_user_menu.label') }}" type="button"
                class="size-9.5 flex items-center justify-center rounded-lg transition-all hover:bg-gray-100">
                <x-filament-panels::avatar.user :user="$user" class="size-7" />
            </button>
        @elseif ($position === UserMenuPosition::Topbar)
            <button aria-label="{{ __('filament-panels::layout.actions.open_user_menu.label') }}" type="button"
